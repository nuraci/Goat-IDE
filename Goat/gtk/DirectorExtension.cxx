...
bool DirectorExtension::Initialise(ExtensionAPI *host_) {
	host = host_;
	IF_DEBUG(fdDebug = fopen("/tmp/"APP_NAME".log", "w"))
...
void DirectorExtension::HandleStringMessage(const char *message) {
	...
	for (int i = 0; i < wlMessage.len; i++) {
		...
		if (isprefix(cmd, "closing:")) {
			...
			if (corresp == NULL) {
				fprintf(stderr,APP_NAME" Director: bad request\n");
				return;
			} else {
				...
				if (fdCorrespondent == -1) {
					fdCorrespondent = 0;
					fprintf(stderr,APP_NAME" Director: cannot open result file '%s'\n",corresp);
					return;
				}
				if (fdCorrespondent != 0) {
					...
					size += write(fdCorrespondent, "\n", 1);
					if (size == 0)
						fprintf(stderr,APP_NAME" Director: cannot write pipe name\n");
			...
			if (corresp != NULL) {
				...
				if (fdCorrespondent == -1) {
					fdCorrespondent = 0;
					fprintf(stderr,APP_NAME" Director: cannot open correspondent pipe '%s'\n",corresp);
					return;
				}
			}
			host->Perform(cmd);
