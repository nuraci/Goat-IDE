...
#define GKEY_BS GDK_KEY_BackSpace
#define GKEY_Left GDK_KEY_Left
#define GKEY_Up GDK_KEY_KP_Up
#define GKEY_Right GDK_KEY_Right
#define GKEY_Down GDK_KEY_Down
#define GKEY_Home GDK_KEY_Home
#define GKEY_End GDK_KEY_End
...
#define GKEY_BS GDK_BackSpace
#define GKEY_Left GDK_Left
#define GKEY_Up GDK_Up
#define GKEY_Right GDK_Right
#define GKEY_Down GDK_Down
#define GKEY_Home GDK_Home
#define GKEY_End GDK_End
...
const char appName[] = APP_NAME_TITLE;
...
class SciTEGTK : public SciTEBase, public Worker {
...
protected:
	...
	FilePath AppNameExecutable;
	FilePath AppNameRootDirFileName;
	int icmd;
	int originalEnd;
	int fdFIFO;
	int pidShell;
	...
	virtual void QuestionDialog(const char *str);
	virtual void AskForFileDialog(FilePath directory, const char *filter);
	virtual bool AbbrevDialog();
	virtual void TabSizeDialog();
	virtual bool ParametersDialog(bool modal);

	virtual FilePath GetExeDirectory(char *name);
	virtual const char *GetGoatDefaultDirectory();
	...
	void QuestionCmd();
	void QuestionResponse(int responseID);
	...
	static gint KeyConsolePress(GtkWidget *widget, GdkEventKey *event, SciTEGTK *scitew);
	static gint KeyRelease(GtkWidget *widget, GdkEventKey *event, SciTEGTK *scitew);
	gint Key(GdkEventKey *event);
	gint KeyConsole(GdkEventKey *event);
	...
	gint TabGroupRelease(GtkNotebook *notebook, GdkEventButton *event);
	...
	static gint TabGroupReleaseSignal(GtkNotebook *notebook, GdkEventButton *event, SciTEGTK *scitew) {
		return scitew->TabGroupRelease(notebook, event);
	}
	static gint TabBarScrollSignal(GtkNotebook *, GdkEventScroll *event, SciTEGTK *scitew) {
		return scitew->TabBarScroll(event);
	}

	gint TabGroupScroll(GdkEventScroll *event);

	static gint TabGroupScrollSignal(GtkNotebook *, GdkEventScroll *event, SciTEGTK *scitew) {
		return scitew->TabGroupScroll(event);
	}
public:
	...
	static gboolean PostCallbackConsole(void *ptr);
	// Single instance
	void SetStartupTime(const char *timestamp);
	static gboolean DoItLater(SciTEGTK *scite);
};
...
FilePath SciTEGTK::GetExeDirectory(char *name) {
	static FilePath Executable = "";

	if (Executable == "") {
		// Find the AppNane executable, first trying to use argv[0] and converting
		// to an absolute path and if that fails, searching the path.
		Executable = FilePath(name).AbsolutePath();
		if (!Executable.Exists()) {
			gchar *progPath = g_find_program_in_path(name);
			Executable = FilePath(progPath);
			g_free(progPath);
		}
	}
...
const char *SciTEGTK::GetGoatDefaultDirectory() {
	static string root;

	if (root.empty()) {
		root = GetExeDirectory(ExeName).Directory().AsUTF8();
		root.erase(root.find_last_of("/"),root.length());
		root.append("/"GOAT_DEFAULT_HOME);
	}
	return root.c_str();
}

FilePath SciTEGTK::GetDefaultDirectory() {
	const static char *where = getenv("GOAT_HOME");
	string root;
#ifdef SYSCONF_PATH
	if (!where) {
		where = SYSCONF_PATH;
...
FilePath SciTEGTK::GetSciteDefaultHome() {
	const char *where = getenv("GOAT_HOME");
#ifdef SYSCONF_PATH
	if (!where) {
		where = SYSCONF_PATH;
	}
...
void SciTEGTK::SizeContentWindows() {
	...
	if (!userStrip.VisibleHasFocus()) {
		...
		if (heightOutput == max) {
			if (wGroupTabCurrentPage == GOA_CON_TARGET)
	...
	if (!dlgFileSelector.Created()) {
		...
		if (props.GetInt("open.dialog.in.file.directory")) {
			gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(dlg),
			                                    // NUCCIO maybe a bug here: filePath.Directory().AsInternal());
			                                    directory.AsInternal());
		}
...
void SciTEGTK::FindInFilesCmd() {
	...
	if (findCommand == "") {
		findCommand = AppNameExecutable.AsInternal();
		findCommand += " -grep ";
...
void SciTEGTK::ContinueExecute(int fromPoll) {
	...
	if (count > 0) {
		...
		if (exitStatus && wGroupTab.GetID()) {
			gtk_notebook_set_current_page(GTK_NOTEBOOK(wGroupTab.GetID()), GOA_CON_HOST);
		}
...
void SciTEGTK::QuestionCmd() {
	const GUI::gui_char *paramVal = dlgQuestion.entryQuestion.Text();
	props.Set("Answer", paramVal);
	dlgQuestion.Destroy();
}

void SciTEGTK::QuestionResponse(int responseID) {
	switch (responseID) {
	case GTK_RESPONSE_OK:
		QuestionCmd() ;
		break;
...
void SciTEGTK::QuestionDialog(const char *str) {

	dlgQuestion.Create(localiser.Text(str));

	gtk_container_set_border_width(GTK_CONTAINER(PWidget(dlgQuestion)), 0);

	WTable table(1, 2);
	table.PackInto(GTK_BOX(dlgQuestion.ContentArea()));

	GtkWidget *labelQuestion = TranslatedLabel(str);
	table.Label(labelQuestion);

	dlgQuestion.entryQuestion.Create();
	table.Add(dlgQuestion.entryQuestion);
	dlgQuestion.entryQuestion.ActivatesDefault();
	gtk_widget_grab_focus(dlgQuestion.entryQuestion);
	gtk_label_set_mnemonic_widget(GTK_LABEL(labelQuestion), dlgQuestion.entryQuestion);

	AttachResponse<&SciTEGTK::QuestionResponse>(PWidget(dlgQuestion), this);
	dlgQuestion.ResponseButton(localiser.Text("_Cancel"), GTK_RESPONSE_CANCEL);
	dlgQuestion.ResponseButton(localiser.Text("_Execute"), GTK_RESPONSE_OK);
	gtk_dialog_set_default_response(GTK_DIALOG(PWidget(dlgQuestion)), GTK_RESPONSE_OK);

	dlgQuestion.Display(PWidget(wSciTE));
}

void SciTEGTK::AskForFileDialog(FilePath directory, const char *f) {
	directory.SetWorkingDirectory();
	if (!dlgFileSelector.Created()) {
		GtkWidget *dlg = gtk_file_chooser_dialog_new(
		                     localiser.Text("Open File").c_str(),
...
static KeyToCommand kmap[] = {
	...
	{m__, GKEY_BS, IDK_BS},
	{m__, GKEY_Up, IDK_UP},
	{m__, GKEY_Down, IDK_DOWN},
	{m__, GKEY_Left, IDK_LEFT},
	{m__, GKEY_Right, IDK_RIGHT},
	{m__, GKEY_Home, IDK_HOME},
	{m__, GKEY_End, IDK_END},
	{m_C, 'a', IDK_CTRL_A},
	{m_C, 'b', IDK_CTRL_B},
	{m_C, 'c', IDK_CTRL_C},
	{m_C, 'd', IDK_CTRL_D},
...
gint SciTEGTK::Key(GdkEventKey *event) {
	...
	if (wConsole.HasFocus() && this->serial->IsConnected() && commandID >= IDK_FIRST && commandID <= IDK_LAST) {
		switch (commandID) {
		case IDK_BS:
			this->serial->Send("\x08",1);
			break;
		case IDK_UP:
			this->serial->Send("\e[A",3);
			break;
		case IDK_DOWN:
			this->serial->Send("\e[B",3);
			break;
		case IDK_LEFT:
			this->serial->Send("\e[D",3);
			break;
		case IDK_RIGHT:
			this->serial->Send("\e[C",3);
			break;
		case IDK_HOME:
			this->serial->Send("\e[H",3);
			break;
		case IDK_END:
			this->serial->Send("\e[F",3);
			break;
		case IDK_CTRL_A:
			this->serial->Send("\x01",1);
			break;
		case IDK_CTRL_B:
			this->serial->Send("\x02",1);
			break;
		case IDK_CTRL_C:
			this->serial->Send("\x03",1);
			break;
		case IDK_CTRL_D:
			this->serial->Send("\x04",1);
			break;
		case IDK_CTRL_E:
			this->serial->Send("\x05",1);
			break;
		}
	...
	if (event->button == 3) {
		...
		if (WindowFromWidget(PWidget(*w)) != event->window) {
			if (WindowFromWidget(PWidget(wOutput)) == event->window) {
				...
				w = &wOutput;
			} else if (WindowFromWidget(PWidget(wConsole)) == event->window) {
				menuSource = IDM_CONSOLEWIN;
				w = &wConsole;
			} else {
				menuSource = 0;
...
gint SciTEGTK::TabGroupRelease(GtkNotebook *notebook, GdkEventButton *event) {
	return FALSE;
}
...
gint SciTEGTK::TabGroupScroll(GdkEventScroll *event) {
	switch (event->direction) {
	case GDK_SCROLL_RIGHT:
	case GDK_SCROLL_DOWN:
		//WindowSetFocus(wEditor);
...
void SciTEGTK::CreateMenu() {
	...
	SciTEItemFactoryEntry menuItemsHelp[] = {
		...
		{"/Help/"APP_NAME" Help" , "", menuSig, IDM_HELP_GOAT, 0},
		{"/Help/About " APP_NAME, "", menuSig, IDM_ABOUT, 0},
	};
...
void SciTEGTK::LayoutUI() {
	...
	if (splitPane) {
		...
		focusConsole = wConsole.HasFocus();
		gtk_container_remove(GTK_CONTAINER(splitPane), PWidget(wEditor));
		gtk_container_remove(GTK_CONTAINER(splitPane), PWidget(wGroupTab));
...
#endif
		gtk_widget_set_size_request(PWidget(wGroupTab), heightOutput, -1);
	} else {
#if GTK_CHECK_VERSION(3,0,0)
...
#endif
		gtk_widget_set_size_request(PWidget(wGroupTab), -1, heightOutput);
	}
	...
	gtk_paned_pack2(GTK_PANED(splitPane), PWidget(wGroupTab), FALSE, TRUE);
...
void SciTEGTK::CreateUI() {
	...
	if (props.GetInt("save.position")) {
		...
		if (splitVertical)
			heightOutput = propsSession.GetInt("output.vertical.size", useDefault);
		else
			heightOutput = propsSession.GetInt("output.horizontal.size", useDefault);
	...
	// The Notebook (GTK2)
	wGroupTab = gtk_notebook_new();
	WIDGET_SET_NO_FOCUS(PWidget(wGroupTab));

	g_signal_connect(G_OBJECT(PWidget(wGroupTab)),
	                 "button-release-event", G_CALLBACK(TabGroupReleaseSignal), gthis);
	g_signal_connect(G_OBJECT(PWidget(wGroupTab)),
	                 "scroll-event", G_CALLBACK(TabGroupScrollSignal), gthis);
	gtk_notebook_set_scrollable(GTK_NOTEBOOK(PWidget(wGroupTab)), TRUE);
	...
	wConsole.SetID(scintilla_new());
	g_object_ref(G_OBJECT(PWidget(wConsole)));
	scintilla_set_id(SCINTILLA(PWidget(wConsole)), IDM_CONSOLEWIN);
	wConsole.Call(SCI_USEPOPUP, 0);
	g_signal_connect(G_OBJECT(PWidget(wConsole)), SCINTILLA_NOTIFY,
	                 G_CALLBACK(NotifySignal), this);
	...
	GtkWidget *tablabel1   = gtk_label_new(localiser.Text("Target Console").c_str());
	GtkWidget *tabcontent1 = PWidget(wConsole);
	GtkWidget *tablabel2   = gtk_label_new(localiser.Text("Host Console").c_str());
	GtkWidget *tabcontent2 = PWidget(wOutput);

	gtk_widget_show(tablabel1);
	gtk_widget_show(tabcontent1);
	gtk_widget_show(tablabel2);
	gtk_widget_show(tabcontent2);

	gtk_notebook_append_page(GTK_NOTEBOOK(wGroupTab.GetID()), tabcontent1, tablabel1);
	gtk_notebook_append_page(GTK_NOTEBOOK(wGroupTab.GetID()), tabcontent2, tablabel2);
...
void SciTEGTK::SetIcon() {
	GdkPixbuf *pixbufIcon = gdk_pixbuf_new_from_xpm_data(Goat_icon_xpm);
...
bool SciTEGTK::PerformOnNewThreadOnConsole(Worker *pWorker) {
	pthread_t tid;
	pthread_create(&tid, NULL, WorkerThreadOnConsole, pWorker);
	return tid != 0;
}
...
bool SciTEGTK::CheckForRunningInstance(int argc, char *argv[]) {
	...
	GPatternSpec *pattern = g_pattern_spec_new(APP_NAME".*.in");
...
void SciTEGTK::Run(int argc, char *argv[]) {
	...
	// Find the AppName executable, first trying to use argv[0] and converting
	// to an absolute path and if that fails, searching the path.
	AppNameExecutable = FilePath(argv[0]).AbsolutePath();
	if (!AppNameExecutable.Exists()) {
		gchar *progPath = g_find_program_in_path(argv[0]);
		AppNameExecutable = FilePath(progPath);
		g_free(progPath);
	}
	...
	// XXX MAIN() is here
	if (props.Get("serial.port").size() != 0)
		serial->SetPort(props.Get("serial.port").c_str());
	else
		serial->SetPort();

	serial->SetSpeed(props.GetInt("serial.speed",115200));
	...
	if (props.Get("serial.eol").size() != 0) {
		if (strcmp(props.Get("serial.eol").c_str(), "CRLF") == 0) {
			serial->SetEOL(UART_EOL_CRLF);
		} else if (strcmp(props.Get("serial.eol").c_str(), "CR") == 0) {
			serial->SetEOL(UART_EOL_CR);
		} else {
			serial->SetEOL(UART_EOL_LF);
		}
	}

	term.SetSerial(serial);
	term.SetTermWin(&wConsole, props.GetInt("term.columns") ,props.GetInt("term.lines"));
	serial->SetTargetConsoleWin(&wConsole);
...
gboolean SciTEGTK::DoItLater(SciTEGTK *scite) {
	...
	if (scite->serial->Start()) {
		if ((scite->props.Get("serial.tx1eol").size() != 0) && scite->props.GetInt("serial.tx1eol") == 1) {
			if (scite->props.Get("serial.eol").size() != 0) {
				/* Send the first EOL in order to get Target prompt */
				if (strcmp(scite->props.Get("serial.eol").c_str(),"CRLF") == 0) {
					scite->serial->OutByte('\r');
					scite->serial->OutByte('\n');
				} else if (strcmp(scite->props.Get("serial.eol").c_str(),"CR") == 0) {
					scite->serial->OutByte('\n');
				} else {
					scite->serial->OutByte('\r');
				}
	...
	if ((scite->props.Get("term.terminal").size() != 0) && scite->props.GetInt("term.terminal") == 1) {
		scite->term.setTermActiveOn();
		scite->term.Clear();
	}
