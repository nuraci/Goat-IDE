...
set abuild_release_date=April 20, 2011
...
set abuild_promptForUpload=yes
...
call "!GOAT_DEFAULT_HOME!\wiring\windows-cli\agetpref.bat" agetpref_internal
if not !errorlevel! == 0 (
    goto end
)
...
if /i "!abuild_SketchName:~-4!" == ".pde" (
    ...
    >>!abuild_cppname! echo.#line 1 ^"!abuild_SketchName!^"
	>>!abuild_cppname! type !abuild_SketchName!
	set arduino_runtime=!arduino_runtime_pde!
	...
	if /i "!abuild_SketchName:~-4!" == ".ino" (
		REM     If we see .ino on the end, we will do a tiny amount of preprocessing,
		REM     but not as much as the Arduino IDE would do.
		set abuild_cppname=!abuild_output!\!abuild_SketchName!.cpp
		set abuild_preprocess=true
		!abuild_report! found PDE file; will do minor preprocessing ==^>  !abuild_cppname!
		...
		>>!abuild_cppname! echo.#line 1 ^"!abuild_SketchName!^"
		>>!abuild_cppname! type !abuild_SketchName!
		set arduino_runtime=!arduino_runtime_ino!
		set arduino_variants=!arduino_path!\hardware\arduino\variants\!arduino_variants!
		set arduino_avr_include=!arduino_path!\hardware\tools\avr\avr\include\avr
		set arduino_defines=-DARDUINO^=100
		if exist "!arduino_runtime!\main.cxx" (
			REM     Getting here means we are using the library optimization patch.
			>>!abuild_cppname! type "!arduino_runtime!\main.cxx"
		)
	) else (
		!abuild_report! found non-PDE/INO file; will not preprocess.
	...
	for /R /D %%d in ("*.*") do (
		...
		if NOT !test! EQU examples (
			for %%f in (!ARDUINO_LIBS!) do (
				rem echo %%d "  " !abuild_include_paths_root!\%%f
				if !abuild_include_paths_root!\%%f EQU %%d (
...
set abuild_gcc_opts=-c -g -Os "-I!arduino_runtime!" "-I!arduino_variants!" "-I!arduino_avr_include!" !abuild_include_paths! -mmcu=!arduino_mcu! !arduino_defines! -DF_CPU=!arduino_fcpu! -DABUILD_BATCH=1
...
if !abuild_nolibs! == false (
		pushd !arduino_path!\libraries
		for /D %%f in ("*") do (
			for %%l in (!ARDUINO_LIBS!) do (
				if %%l EQU %%f (
					!abuild_report! Building library: %%f
					popd
					set abuild_objfile=!abuild_output!\%%~nf.cpp.o
					set abuild_cmd=avr-g++ !abuild_gpp_opts! "!arduino_path!\libraries\%%~f\%%~f.cpp" "-o!abuild_objfile!"
					!abuild_report! !abuild_cmd!
					if exist !abuild_objfile! (del !abuild_objfile!)
					call !abuild_cmd!
					if not exist !abuild_objfile! (goto end)
...
if not !abuild_promptForUpload! == true (
    ...
    call "!GOAT_DEFAULT_HOME!\wiring\windows-cli\aupload.bat" !abuild_hex!
    if not !errorlevel! == 0 (
