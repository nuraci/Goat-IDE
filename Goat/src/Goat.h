// Goat - Scintilla based IDE
/** @file Goat.h
 ** Define command IDs used within Goat.
 **/
...
#ifdef DEBUG
#ifdef WIN32
void __cdecl myDbgPrint(const char *s, ...);
#define MYDEBUG(__f, ...) myDbgPrint("%s: %s() @ %d ["__f"]\n",__FILE__, __FUNCTION__,__LINE__, ## __VA_ARGS__ )
#else
#define MYDEBUG(__f, ...) printf("%s: %s() @ %d ["__f"]\n",__FILE__, __FUNCTION__,__LINE__, ## __VA_ARGS__ )
#endif
#else
#define MYDEBUG
#endif
