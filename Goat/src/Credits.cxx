...
void SciTEBase::SetAboutMessage(GUI::ScintillaWindow &wsci, const char *appTitle) {
	if (wsci.Created()) {
		...
		AddStyledText(wsci, " Nuccio Raciti  ", 1);
  	    AddStyledText(wsci, "(raciti.nuccio@gmail.com)", 1);
  	    AddStyledText(wsci, "\n", 1);
		...
		AddStyledText(wsci, GetTranslationToAbout("for his valuable work on Scintilla and SciTE:").c_str(), trsSty);
		AddStyledText(wsci, "\n\n", 2);
#if !defined(GTK)
...
#endif
		AddStyledText(wsci, APP_NAME, trsSty);
		AddStyledText(wsci, GetTranslationToAbout(" is based on Scintilla and SciTE:\n").c_str(), trsSty);
		AddStyledText(wsci, GetTranslationToAbout("Version").c_str(), trsSty);
