...
int SciTEBase::CallFocused(unsigned int msg, uptr_t wParam, sptr_t lParam) {
	if (wOutput.HasFocus())
		...
		return wConsole.Call(msg, wParam, lParam);
	else
...
sptr_t SciTEBase::CallPane(int destination, unsigned int msg, uptr_t wParam, sptr_t lParam) {
	if (destination == IDM_SRCWIN)
		...
		return wOutput.Call(msg, wParam, lParam);
	else if (destination == IDM_CONSOLEWIN)
		return wConsole.Call(msg, wParam, lParam);
	else
...
void SciTEBase::CallChildren(unsigned int msg, uptr_t wParam, sptr_t lParam) {
	...
	wConsole.Call(msg, wParam, lParam);
}
...
void SciTEBase::BraceMatch(bool editor) {
	...
	GUI::ScintillaWindow &win = editor ? wEditor : wOutput.HasFocus() ? wOutput : wConsole;
...
/*
 * Upon a character being added to the console, SciTE may decide to perform some action
 * such as displaying a completion list or running a shell command.
 */
void SciTEBase::CharAddedConsole(int ch) {
	int pos = wConsole.Send(SCI_GETCURRENTPOS);
	wConsole.Send(SCI_SETTARGETSTART, pos-1);
	wConsole.Send(SCI_SETTARGETEND, pos);
	wConsole.Send(SCI_REPLACETARGET, 0, reinterpret_cast<sptr_t>(""));

	if (ch == '\n' || ch == '\r') {
		if (serial->GetEOL() == UART_EOL_CRLF) {
			serial->OutByte(UART_EOL_CR);
			serial->OutByte(UART_EOL_LF);
		} else if (serial->GetEOL() == UART_EOL_CR) {
			serial->OutByte(UART_EOL_CR);
		} else {
			serial->Send((const char *)&ch,1);
		}
	} else {
		serial->Send((const char *)&ch,1);
	}
...
void SciTEBase::GoMatchingBrace(bool select) {
	int braceAtCaret = -1;
	int braceOpposite = -1;
	GUI::ScintillaWindow *wCurrent = &wEditor;
	bool OutputHasFocus;
	bool isInside;
	...
	if (wOutput.HasFocus()) {
		...
		wCurrent = &wEditor;
		OutputHasFocus = false;
	}
	isInside = FindMatchingBracePosition(!OutputHasFocus, braceAtCaret, braceOpposite, true);
...
void SciTEBase::MenuCommand(int cmdID, int source) {
	...
	case IDM_EXAMPLE_OPEN:
		// No need to see if can make room as that will occur
		// when doing the opening. Must be done there as user
		// may decide to open multiple files so do not know yet
		// how much room needed.
		OpenDialog(FilePath(GUI::StringFromUTF8(props.Get(EXAMPLES_DIR_P).c_str()).c_str()),
	...
	case IDM_BUILD: {
		...
		break;
	case IDM_MODE_TERMINAL: {
			if (term.isTermActive())
				term.setTermActiveOff();
			else
				term.setTermActiveOn();
			CheckMenus();
			term.Clear();
			WindowSetFocus(wConsole);
		}
	...
	case IDM_OPEN_UART:
		if (serial->Start()) {
			...
			CheckMenus();
			term.Clear();
			if ((props.Get("serial.tx1eol").size() != 0) && props.GetInt("serial.tx1eol") == 1) {
				/* Send the first EOL in order to get Target prompt */
				if (serial->GetEOL() == UART_EOL_CRLF) {
					serial->SendCrlf();
				} else {
					serial->OutByte(serial->GetEOL());
				}
			}
			GroupSetCurrentTab(GOA_CON_TARGET);
		}
...
void SciTEBase::Notify(SCNotification *notification) {
	...
	case SCN_CHARADDED:
		...
		if (!handled) {
			...
			} else if (notification->nmhdr.idFrom == IDM_CONSOLEWIN) {
				CharAddedConsole(notification->ch);
			} else {
...
void SciTEBase::UIAvailable() {
	...
	if (extender) {
		...
		props.Set(APP_NAME"DefaultHome", homepath.AsUTF8().c_str());
...
void SciTEBase::PerformOne(char *action) {
	...
	if (arg) {
		...
		} else if (isprefix(action, "console:") && wConsole.Created()) {
			wConsole.Call(SCI_REPLACESEL, 0, reinterpret_cast<sptr_t>(arg));
...
void SciTEBase::Remove(Pane p, int start, int end) {
	...
	case paneOutput:
		...
		break;
	case paneConsole:
		wConsole.Call(SCI_SETSEL, start, end);
		wConsole.Call(SCI_CLEAR);
		break;
	}
}
...
void SciTEBase::Insert(Pane p, int pos, const char *s) {
	...
	case paneOutput:
		...
		break;
	case paneConsole:
		wConsole.CallString(SCI_INSERTTEXT, pos, s);
		break;
	}
}
...
int SciTEBase::SerialSendFile(const char *name, int type) {

	if (serial->IsConnected()) {
		return serial->SerialTrasmitFile(name, type);
	}
	return -1;
}

int SciTEBase::SerialSend(const char *string, int length) {
	if (serial->IsConnected()) {
		return serial->Send(string, length);
	}
	return -1;
}
void SciTEBase::SelectConsoleTab(int tab) {
	if (wGroupTab.GetID()) {
		GroupSetCurrentTab(tab);
		WindowSetFocus(wConsole);
	}
}
void SciTEBase::AskForFile(const char *dirName, const char *filter) {
	AskForFileDialog(FilePath(GUI::StringFromUTF8(dirName).c_str()),GUI::StringFromUTF8(filter).c_str());
}
void SciTEBase::AskQuestion(const char *str) {
	QuestionDialog(str);
}
