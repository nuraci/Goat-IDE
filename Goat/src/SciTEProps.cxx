...
const GUI::gui_char propDirectoryFileName[] = GUI_TEXT("GoatDirectory.properties");
...
void SciTEBase::ReadDirectoryPropFile() {
	...
	if (props.GetInt("properties.directory.enable") != 0) {
		...
		props.Set(APP_NAME"DirectoryHome", propfile.Directory().AsUTF8().c_str());
...
void SciTEBase::ForwardPropertyToEditor(const char *key) {
	if (props.Exists(key)) {
		...
		wOutput.CallString(SCI_SETPROPERTY,
						 reinterpret_cast<uptr_t>(key), value.c_str());
		wConsole.CallString(SCI_SETPROPERTY,
						 reinterpret_cast<uptr_t>(key), value.c_str());
	}
}
...
void SciTEBase::ReadProperties() {
	...
	scrollConsole = props.GetInt("console.scroll", 1);
...
void SciTEBase::ReadPropertiesInitial() {
	...
	int sizeHorizontal = props.GetInt("output.horizontal.size", 100);
	int sizeVertical = props.GetInt("output.vertical.size", 100);
