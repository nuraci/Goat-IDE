...
protected:
	...
	virtual void AskForFileDialog(FilePath directory, const GUI::gui_char *filter);
	...
	virtual const GUI::gui_char *GetGoatDefaultDirectory();
	...
	BOOL QuestionMessage(HWND hDlg, UINT message, WPARAM wParam);
	static BOOL CALLBACK GoLineDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
	static BOOL CALLBACK QuestionDlg(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
...
public:
	...
	static void WriteOnConsole(char *s, unsigned int len);
	static DWORD WINAPI DoItLater(LPVOID lparam);
	int GroupTabSelected;
};
